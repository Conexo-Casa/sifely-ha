...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    ...
    async def handle_add_passcode(call: ServiceCall) -> None:
        ...
        try:
            result = await client.add_passcode(lock_id, call.data[ATTR_PASSCODE],
            ...
            hass.bus.async_fire(f"{DOMAIN}_passcode_added", {
                "lock_id": lock_id, "passcode_id": result.get("keyboardPwdId"),
                "passcode": result.get("keyboardPwd"), "passcode_name": call.data[ATTR_PASSCODE_NAME],
            })
    ...
    async def handle_change_passcode(call: ServiceCall) -> None:
        ...
        try:
            await client.change_passcode(lock_id, pid,
                call.data.get(ATTR_PASSCODE), call.data.get(ATTR_PASSCODE_NAME),
                call.data.get(ATTR_START_DATE), call.data.get(ATTR_END_DATE))
            hass.bus.async_fire(f"{DOMAIN}_passcode_changed", {"lock_id": lock_id, "passcode_id": pid})
        except SifelyApiError as err:
    ...
    async def handle_delete_passcode(call: ServiceCall) -> None:
        ...
        try:
            ...
            hass.bus.async_fire(f"{DOMAIN}_passcode_deleted", {"lock_id": lock_id, "passcode_id": pid})
        except SifelyApiError as err:
    ...
    async def handle_list_passcodes(call: ServiceCall) -> None:
        ...
        try:
            ...
            hass.bus.async_fire(f"{DOMAIN}_passcodes_listed", {"lock_id": lock_id, "passcodes": passcodes})
        except SifelyApiError as err:
